...
exports.Printer = exports.TerminalBuffer = exports.Slot = exports.Attribute = exports.CompositeMode = exports.Color = exports.ColorMode = void 0;
const unicode11_1 = require("./unicode11");
var ColorMode;
(function (ColorMode) {
    ColorMode[ColorMode["Default"] = 0] = "Default";
    ColorMode[ColorMode["Palette"] = 1] = "Palette";
    ColorMode[ColorMode["Real"] = 2] = "Real";
    /**
     ...
     * */
    ColorMode[ColorMode["Invalid"] = 3] = "Invalid";
...
(function (Color) {
    ...
    Color[Color["blackBright"] = 8] = "blackBright";
    Color[Color["redBright"] = 9] = "redBright";
    Color[Color["greenBright"] = 10] = "greenBright";
    Color[Color["yellowBright"] = 11] = "yellowBright";
    Color[Color["blueBright"] = 12] = "blueBright";
    Color[Color["magentaBright"] = 13] = "magentaBright";
    Color[Color["cyanBright"] = 14] = "cyanBright";
    Color[Color["whiteBright"] = 15] = "whiteBright";
...
(function (CompositeMode) {
    ...
    CompositeMode[CompositeMode["OverrideNone"] = 0] = "OverrideNone";
    /** override the original slot even if content is null */
    CompositeMode[CompositeMode["OverrideContent"] = 1] = "OverrideContent";
    /** override the original slot style even if style are default */
    CompositeMode[CompositeMode["OverrideStyle"] = 2] = "OverrideStyle";
    /** (Default) override the original slot even if content is null or style are default */
    CompositeMode[CompositeMode["OverrideBoth"] = 3] = "OverrideBoth";
})(CompositeMode = exports.CompositeMode || (exports.CompositeMode = {}));
class Attribute {
    colorForegroundMode;
    colorBackgroundMode;
    colorForeground = 0;
    colorBackground = 0;
    constructor(colorForegroundMode = ColorMode.Default, colorForeground = 0, colorBackgroundMode = ColorMode.Default, colorBackground = 0) {
        this.colorForegroundMode = colorForegroundMode;
        this.colorBackgroundMode = colorBackgroundMode;
    ...
    with({ colorForegroundMode, colorForeground, colorBackgroundMode, colorBackground } = {}) {
        const newAttr = new Attribute(colorForegroundMode ?? this.colorForegroundMode, colorForeground ?? this.colorForeground, colorBackgroundMode ?? this.colorBackgroundMode, colorBackground ?? this.colorBackground);
        return newAttr;
    }
    isValid() {
    ...
    static from({ colorForegroundMode = ColorMode.Default, colorForeground = 0, colorBackgroundMode = ColorMode.Default, colorBackground = 0 } = {}) {
        const attr = new Attribute(colorForegroundMode, colorForeground, colorBackgroundMode, colorBackground);
        return attr;
    }
    mixWith(attr) {
        ...
        return new Attribute(fgm, fg, bgm, bg);
    }
...
class TerminalBuffer {
    ...
    height;
    grid;
    static unicode = new unicode11_1.UnicodeV11;
    nullFillCharacter = '';
    ...
    static lengthOf(str) {
        return [...str].map(c => this.unicode.wcwidth(c.codePointAt(0))).reduce((a, b) => a + b, 0);
    }
    constructor(width, height) {
        this.width = width;
        this.height = height;
        this.grid = new Array(height).fill(this.nullFillCharacter).map(() => new Array(width).fill(this.nullFillCharacter).map(() => new Slot()));
    }
    resize(width, height) {
     ...
     */
    write(row, col, text, attr, boundStart = -Infinity, boundEnd = Infinity) {
        if (text === '')
            return 0;
        ...
        if (col + actualLength <= 0)
            return actualLength;
        // double width head fix
        if (this.grid[row][realBoundStart] &&
        ...
        if (this.grid[row][col + actualLength - 1] &&
     ...
     */
    fill(dy, dx, h, w, text, attr, mode = CompositeMode.OverrideBoth) {
        if (attr ? !attr.isValid() : false)
            throw new Error('invalid color');
        for (let r = 0; r < h; r++) {
     ...
     * @returns Diff
     */
    diff(toBuf, fromY, fromX, toY, toX, h, w) {
        let res = '';
        let currentCursorStyle = Attribute.from({
            colorBackgroundMode: ColorMode.Invalid
        });
        ...
        for (let r = 0; r < h; r++) {
            ...
            let nullFill = 0;
            for (let c = 0; c < w; c++) {
                let oldSlot = toBuf.grid[r + toY]?.[c + toX] || defaultSlot;
                let newSlot = this.grid[r + fromY]?.[c + fromX] || defaultSlot;
                if (c === 0) {
                    if (oldSlot.length === 0) {
                        const prev = oldSlot;
                        oldSlot = new Slot();
...
class Printer extends TerminalBuffer {
    currentScreen;
    constructor(width, height) {
        super(width, height);
