...
const scrollBuf = new terminal_1.TerminalBuffer(tty.columns + panelWidth + gap, tty.rows);
const subBuf = new terminal_1.TerminalBuffer(tty.columns, 1);
...
async function main() {
    ...
    const strLength = terminal_1.TerminalBuffer.lengthOf(str);
    ...
    while (!tty.writableEnded) {
        ...
        if (offset === 0) {
            ...
            for (let i = 0; i < 2; i++) {
                const attr = terminal_1.Attribute.from({
